...
<table  class="table table-bordered " style="font-size: 14px; width:100%;">
 ...
 $gtotal1=0;
    ...
    @foreach($customer as $k=>$rowData)
                ...
                <td colspan="6">
                    <table style="width: 100%">
                        <tr>
                            ...
                            <td style="padding: 0px;"><strong>Rent Rate: {{$rowData['bill2']['rate']??""}}</strong></td>
                        ...
                        <tr>
                             <td style="padding: 0px;"><strong>Meter No: </strong></td>
                              <td style="padding: 0px;"><strong>Bill Start From: {{$rowData['bill2']['date_s']??""}}</strong></td>
                           
                            <td style="padding: 0px;"><strong>Area: {{$rowData['bill2']['area_sft']??""}}</strong></td>
                        </tr>
        ...
        @php
            $total=$row['bill_amount']+$row['fine_amount']+$row['fixed_fine'];
            if($total==0){
                continue;
            }
            $gtotal += $total;
                  $total1 += $row['bill_amount'];
            $fine += $row['fine_amount']+$row['fixed_fine'];
    ...
    @endforeach
        ...
        $gamounttotal +=$total1;
            ...
            <tr>
                ...
                <td style="text-align: right">
                    <strong>  {{ number_format($fine) }}  </strong>
                </td>
                <td style="text-align: right">
    ...
    <tr>
        ...
        <td style="text-align: right">
            ...
            <strong> {{ number_format($gamounttotal) }}    </strong>
        </td>

        <td style="text-align: right">
            <strong>  {{ number_format($gfinetotal) }}  </strong>
        </td>
        <td style="text-align: right">
            <strong>   {{ number_format($gtotal1) }}  </strong>
        </td>
    </tr>
