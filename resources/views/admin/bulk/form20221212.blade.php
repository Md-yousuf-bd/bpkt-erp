...
<table id="assetInfo" class="table table-bordered table-hover table-striped" style="font-size: 14px; width:100%;">
    ...
    @foreach($customer as $row)
        ...
        $grand_total=0;
        $bill_amount=0;
                        $grand_total = $row->rate*$row->area;
                        $bill_amount = $row->rate*$row->area;
                    ...
                    if ($type == 29) {
                      ...
                      //  $ass = Asset::where('asset_no',$row->asset_no)->first();
                        if(date('Y-m',strtotime($row->date_s))==date('Y-m',strtotime($bill_date))){
                            $tardate = date('Y-m-'.$d,strtotime($bill_date));
                            $termiante = date('Y-m-'.$d,strtotime($bill_date));
                            $cmpdate = $row->date_s;
    ...
    color: #fff;"> Bill Done</span>
            ...
            @if($type==29)
                <td> {{ $row->rate }} <input type="hidden" name="rate[]" value="{{$row->rate}}">
                </td>
