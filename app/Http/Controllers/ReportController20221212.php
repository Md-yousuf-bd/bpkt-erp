        ...
        if ($request->isMethod('post')) {
            try {
                ...
                $result = Billing:: where('billings.payment_status', '=', $paidDues)
                    ->when($selected_customer != '', function ($query) use ($selected_customer) {
                        return $query->where('billings.customer_id', $selected_customer);
                    })
                    ->when($shop_no != '', function ($query) use ($shop_no) {
                    ...
                    ->when($date_s != '', function ($query) use ($date_s) {
                        return $query->where('billings.issue_date', '>=', $date_s);
                    })
                    ->when($date_e != '', function ($query) use ($date_e) {
                        return $query->where('billings.issue_date', '<=', $date_e);
                    })
                    ->select('billings.*',
...
//return $result;
    ...
    public function duesReport(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                $ar = Billing::
                whereIn('billings.shop_no',$array)
                    ->when($bill_type!='', function ($query) use ($bill_type) {
                        return $query->where('billings.bill_type', '=', $bill_type);
                    })
                    ...
                    sum(billing_details.amount) as bill_amount,
  ...
  END, MONTH ASC")
                ...
                foreach ($ar as $row) {
                    ...
                    if (isset($result[$row['month']])) {
                        ...
                        $result[$row['month']]['fine_amount'] += $row['bill_type'] == 'Service Charge' ? $row['fine_amount'] : 0;
                        $result[$row['month']]['fixed_fine'] += $row['bill_type'] == 'Service Charge' ? $row['fixed_fine'] : 0;
                        ...
                        $result[$row['month']]['sc_fine'] += $row['bill_type'] == 'Service Charge' ? $row['fine'] : 0;
                        ...
                        $result[$row['month']]['el_fine'] += $row['bill_type'] == 'Electricity' ? $row['fine_amount'] : 0;
...
//                $data = ['title' => 'Welcome to Pakainfo.com'];
            ...
            $data['asset'] = Billing::leftjoin('customers', 'customers.id', '=', 'billings.customer_id')->SelectRaw('billings.*,customers.shop_name as name')->orderBy('billings.shop_no', 'ASC')->groupBy('billings.shop_name')->groupBy('billings.shop_no')->get();
    ...
    public function receivableCollectionStatementReport(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (!in_array($checkData['month_to'], $dateFilter)) {
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                leftjoin('cash_collection_details', 'cash_collection_details.ref_id', '=', 'cash_collections.id')
                    ...
                    ->when($service != '', function ($query) use ($service) {
                        return $query->where('item_head_id', '=', $service);
                    })
                    ->SelectRaw('cash_collections.shop_no,cash_collections.shop_name,sum(cash_collections.discount) as discount,
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                leftjoin('billing_details', 'billing_details.billing_id', '=', 'billings.id')
                    ...
                    ->when($date_s != '', function ($query) use ($date_s) {
                        return $query->where('issue_date', '>=', $date_s);
                    })
                    ->when($date_e != '', function ($query) use ($date_e) {
                        return $query->where('issue_date', '<=', $date_e);
                    })
                    ->when($shop_no != '', function ($query) use ($shop_no) {
                        return $query->where('billings.shop_no', '=', $shop_no);
                    })
                    ...
                    ->SelectRaw('billings.shop_no,billings.shop_name,billings.invoice_no,
                       billings.meter_no,billings.bill_type,billing_details.month,billing_details.total,sum(cash_collections.payment_amount) as payment_amount')
                    ->groupBy('billings.invoice_no')
                    ->orderBy('billings.shop_no', 'ASC')
                    ->get();
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                if($deposit_re==1){
                    ...
                    $billings = Journal::when($date_s != '', function ($query) use ($date_s) {
                        return $query->where('effective_date', '>=', $date_s);
                    })
                        ->when($date_e != '', function ($query) use ($date_e) {
                            return $query->where('effective_date', '<=', $date_e);
                        })
                        ->when($shop_no != '', function ($query) use ($shop_no) {
                            return $query->where('shop_no', '=', $shop_no);
                        })
                        ...
                        ->selectRaw('journals.*,sum(credit) as credit')
            ...
            try {
                ...
                foreach ($asset as $r){
                  $billings = AssetLog::where('asset_no', '=', $r['asset_no'])
                        ...
                        ->selectRaw('asset_logs.*,customers.shop_name')
                        ->get();
                  foreach ($billings as $row){
